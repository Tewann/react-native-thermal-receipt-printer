...
export var NetPrinter = {
    ...
    printBill: function (text, opts) {
        return new Promise( async (resolve, reject) => {
            ...
            if (Platform.OS === "ios") {
                var processedText = textPreprocessingIOS(text);
                await RNNetPrinter.printRawData(processedText.text, processedText.opts, function (error) { return reject(error); });
                resolve()
            }
            else {
                await RNNetPrinter.printRawData(billTo64Buffer(text, opts),function() {
                    return resolve()
                }, function (error) {
                    return reject(error);
                });
